test_that("Influcnce is as expected for control polygon",
          {
...
test_that("influence weights for control polygon is as expected",
          {
            bmat <- bsplines(x      = seq(0, 6, length = 500),
                             iknots = c(1.0, 1.5, 2.3, 4.0, 4.5))
            theta <- c(1, 0, 3.5, 4.2, 3.7, -0.5, -0.7, 2, 1.5)
...
test_that("influence_weights for control net are as expected",
          {
            ...
            expected <-
              list(structure(list(iknots = c(-0.927941546989166, -0.864279864279864, 
                                             ...
                                             -0.539154539154539, -0.472361459203564, -0.408850408850409, -0.347189042841217, 
                                             -0.278624338624339, -0.214561500275786, -0.149823633156966, -0.084920634920635, 
                                             -0.0338259441707718, 0.135786435786436, 0.3003663003663, 0.463003663003663, 
                                             0.645058023209284, 0.816829004329004), `max(.data$w)` = c(2.20716057270512, 
                                             2.08089753612485, 1.27994790254683, 0.839986320318741, 0.870404268939342, 
            ...
            iknots = c(46.5030784738986, 46.8052824384244, 47.054023071138, 
                       ...
                       52.0037799690902), `max(.data$w)` = c(0.622589787427728, 
