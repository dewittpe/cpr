...
influence_data <- rbind(
  cbind(setNames(eg_cp$cp, c("x", "y")), object = 1),
  cbind(get_spline(eg_cp)$spline,        object = 2),
  cbind(setNames(omit_xi$coarsened_cp[[1]]$cp, c("x", "y")), object = 3),
  cbind(get_spline(omit_xi$coarsened_cp[[1]])$spline, object = 4),
  cbind(setNames(omit_xi$coarsened_cp[[2]]$cp, c("x", "y")), object = 5),
  cbind(get_spline(omit_xi$coarsened_cp[[2]])$spline, object = 6),
  cbind(setNames(omit_xi$reinserted_cp[[1]]$cp, c("x", "y")), object = 7),
  cbind(get_spline(omit_xi$reinserted_cp[[1]])$spline, object= 8),
  cbind(setNames(omit_xi$reinserted_cp[[2]]$cp, c("x", "y")), object = 9),
  cbind(get_spline(omit_xi$reinserted_cp[[2]])$spline, object = 10)
  )
...
influence_data <-
  rbind(
        cbind(subset(influence_data, object %in% c(1:2, 3:4)), facets = 1),
        cbind(subset(influence_data, object %in% c(1:2, 3:4, 7:8)), facets = 2),
        cbind(subset(influence_data, object %in% c(1:2, 5:6)), facets = 3),
        cbind(subset(influence_data, object %in% c(1:2, 5:6, 9:10)), facets = 4)
        )
...
influence_weight_plot %+%
  ...
  aes(x = x, y = y, color = CP, shape = CP, linetype = CP,
      group = object) +
  geom_point(data = subset(influence_data, object %in% seq(1, 9, by = 2))) +
  geom_line() +
...
ggplot(subset(spdg, id %in% 1:100)) +
  theme_bw() +
...
plot_data <- lapply(list(cpr_run4, cpr_run3, cpr_run2), summary)
for(i in seq_along(plot_data)) {
plot_data[[i]][["order"]] <- i
}
plot_data <- do.call(rbind, plot_data)
plot_data$order <- factor(plot_data$order, 1:3, c("4th", "3rd", "2nd"))
plot_data <- subset(plot_data, index < 13)

ggplot2::ggplot(data = plot_data) +
  ggplot2::theme_bw() +
  ggplot2::aes(x = dfs, y = rmse, color = order, linetype = order) +
  ggplot2::geom_path() +
  ggplot2::geom_point()
@
\caption{}
