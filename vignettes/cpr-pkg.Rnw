...
bmat <- cpr::bsplines(x      = seq(0, 6, length = 500),
                      iknots = c(1.0, 1.5, 2.3, 4.0, 4.5))
@
<<basis_plot, dependson = "eg_bmat", echo = FALSE, results = "hide", fig.width = 2.5, fig.height = 2.5>>=
plot(bmat, show_xi = TRUE, show_x = TRUE, color = TRUE, digits = 1) +
  ggplot2::theme(text = ggplot2::element_text(family = "Times", size = 10))
@
...
eg_cp <- cp(bmat, theta)
@
<<cp_plot, dependson = "eg_bmat", echo = FALSE, results = "hide", fig.width = 2.5, fig.height = 2.5>>=
...
cp_plot0 +
  ggplot2::geom_line(mapping = ggplot2::aes(x = xi_star, y = theta), data = eg_cp$cp,
            inherit.aes = FALSE, linetype = 3) +
  ggplot2::geom_line(mapping = ggplot2::aes(x = x, y = y), data = get_spline(eg_cp)$spline,
            inherit.aes = FALSE) +
  ggplot2::geom_point(mapping = ggplot2::aes(x = xi_star, y = theta),
...
\begin{figure}
  \centering
<<iwp, dependson = c("eg_bmat", "eg_cp.cpr_bs"), echo = FALSE, results = "hide", fig.width = 6, fig.height = 6>>=
...
influence_data <-
  rbind(
        cbind(setNames(eg_cp$cp, c("x", "y")), object = 1L),
        cbind(get_spline(eg_cp)$spline,        object = 2L),
        cbind(setNames(omit_xi$coarsened_cp[[1]]$cp, c("x", "y")), object = 3L),
        cbind(get_spline(omit_xi$coarsened_cp[[1]])$spline,        object = 4L),
        cbind(setNames(omit_xi$coarsened_cp[[2]]$cp, c("x", "y")), object = 5L),
        cbind(get_spline(omit_xi$coarsened_cp[[2]])$spline,        object = 6L),
        cbind(setNames(omit_xi$reinserted_cp[[1]]$cp, c("x", "y")), object = 7L),
        cbind(get_spline(omit_xi$reinserted_cp[[1]])$spline,        object = 8L),
        cbind(setNames(omit_xi$reinserted_cp[[2]]$cp, c("x", "y")), object = 9L),
        cbind(get_spline(omit_xi$reinserted_cp[[2]])$spline,        object = 10L)
        )
...
influence_data <-
  rbind(cbind(subset(influence_data, object %in% c(1:2, 3:4)), facets = 1L),
        cbind(subset(influence_data, object %in% c(1:2, 3:4, 7:8)), facets = 2L),
        cbind(subset(influence_data, object %in% c(1:2, 5:6)), facets = 3L),
        cbind(subset(influence_data, object %in% c(1:2, 5:6, 9:10)), facets = 4L)
        )
...
ggplot2::`%+%`(influence_weight_plot, influence_data) +
  ggplot2::aes(x = x, y = y, color = CP, shape = CP, linetype = CP, group = object) +
  ggplot2::facet_wrap( ~ facets, labeller = ggplot2::label_parsed) +
  ggplot2::geom_point(data = subset(influence_data, object %in% seq(1, 9, by = 2))) +
  ggplot2::geom_line() +
  ggplot2::theme(
        ...
        legend.key.width = ggplot2::unit(0.5, "inch"),
        legend.title     = ggplot2::element_blank()) +
  ggplot2::coord_cartesian(ylim = c(-1.3, 4.8)) +
  ggplot2::theme(text = ggplot2::element_text(family = "Times", size = 10))
@
...
efficiency for several functions and matrix arithmetic.  Two-dimensional
graphics are generated via
...
<<calling_cp.formula>>=
dat <- data.frame(x = seq(-pi, pi, length = 200))
dat$y <- sin(dat$x)
...
<<>>=
str(cpr::influence_of(eg_cp), max.level = 1)
@
...
<<eval = FALSE>>=
plot(cpr::influence_of(eg_cp, c(6, 8)))
@
...
<<>>=
data(list = "spdg", package = "cpr")
str(spdg)
length(unique(spdg$id))
@

\begin{figure}
  \centering
<<spdg_plot, echo = FALSE, fig.width = 3, fig.height = 3>>=
ggplot2::ggplot(subset(spdg, id %in% 1:100)) +
  ggplot2::theme_bw() +
  ggplot2::aes(x = day, y = pdg, group = id) +
...
\Sexpr{length(unique(spdg$id))} subjects.  Subject level variables are
...
plot(cpr_run4, color = TRUE) + ggplot2::theme(legend.position = "bottom") # Figure \ref{fig:cpr_run4_plot1}
@
    \caption{}
...
<<other_cps, eval = FALSE, dependson = "initial_cp4">>=
