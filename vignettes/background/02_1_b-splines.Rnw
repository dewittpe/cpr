...
@
  \caption{Each B-spline multiplied by its coefficient, e.g, $\theta_1 B_{1, k,
    \bs{\xi}} \left( x \right).$
...
@
  ...
  \end{subfigure}
  \caption{basis, element-wise multiplication, and the spline function.  These
...
possible but come at a price.  B-splines are $k$ order polynomials and thus the
spline function $\bs{B}_{k, \bs{\xi}} \left(x \right) \bs{\theta}$ has
$k$ continuity conditions (continuous, 1st order differentiable, 2nd order
differentiable, \ldots $k - 1$ order differentiable) for $x \notin \bs{\xi}.$
The spline function looses one order of continuity for at $x = \xi_j$ for each
multiple of $\xi_j$ in $\bs{\xi}.$  Thus, a cubic (order $k = 4$) spline
function is trice differentiable between interior knots, twice differentiable at
simple (multiplicity one) knots, once differentiable at double knots,
at least continuous at triple knots, and has no continuity condition at knots of
multiple four.  By construction, spline functions built on B-splines have no
continuity construction at the boundary.  

In the above, and moving forward, we will restrict ourselves to simple interior
knots.  While the methods to follow are applicable to splines with interior
knots of higher multiplicity, we opt for this simple approach.  We base this on
the comment from~\citet[page 106]{deboor2001}:
\begin{quote}
  \ldots a B-spline doesn't change much if one changes its $k + 1$ knots
  a little bit.  Therefore, if one has multiple knots, then it is easy to find a
  B-spline almost like it with simple knots: simply replace each knot of
  multiplicity $r > 1$ by $r$ simple knots near by.
\end{quote}
Further, if we assume that the functions we are going to model are smooth, i.e.,
multiply differentiable, we can maintain a smooth functions with simple knots
that are nearly non-smooth due to knots placed near each other.

When trying to determine how many and where to place knots for modeling the
functional relationship between a response and single continuous predictor it
would be helpful to understand how the knot sequence $\bs{\xi}$ and regression
coefficients $\bs{\theta}$ relate to each other.  Such a relationship not not
evident, at least not to me, form the plots in
