...
trivially, for $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta} \in \mathbb{S}_{k, \bs{\xi}}$
implies $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta} \in \mathbb{S}_{k, \bs{\xi}_r}.$
...
$\bs{\xi}_r$ such that $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta} \left(x \right) = f_{s,
k, \bs{\xi}_r, \bs{\theta}_r} \left( x \right).$
...
\begin{Bsproperty}[Knot Sequence Refinement]
  ...
  Then, for any
  $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta} \in \mathbb{S}_{k,
  \bs{\xi}}$ there exists a $\bs{\theta}_r$ such that
  $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta} \left( x \right) =
  f_{s, k, \bs{\xi}_r, \bs{\theta}_r} \left( x \right), \ \forall x \in
  \left[u_{(1)}, u_{\left(\card{\bs{u}}\right)}\right],$ where 
  \begin{equation}
  ...
  \begin{equation}
    ...
    \bs{W}_{k, \bs{\xi}} \left(\zeta \right) = \begin{bmatrix}
      ...
      0                                              & 0 & \cdots & 0      & 1 - \omega_{\card{\bs{\theta}}, k,\bs{\xi}} \left(\zeta \right) & \omega_{\card{\bs{\theta}}, k, \bs{\xi}} \left(\zeta\right) \\
      0                                              & 0                                              & \cdots                                     & 0      & 0                                                                & 1
    \end{bmatrix}
  \end{equation}
  where
...
% <<knot-operations-refine-xi, cache = TRUE, dependson = "b-spline-fundamentals-setup">>=
% @
% 
% <<knot-operations-refine-graphic, cache = TRUE, dependson = c("b-spline-fundamentals-setup", "knot-operations-refine-xi"), fig.scap = scap, fig.cap = cap, fig.width = 6, fig.height = 3, fig.out = "0.98\\textwidth">>=
% @
...
based on the original knot sequence $\bs{\xi}.$
...
Figure~\ref{fig:knot-operations-refine-graphic} show the result of
inserting $\zeta = Sexpr{zeta}$ in the knot sequence used in
Figure~\ref{fig:b-spline-fundamentals-basis-plots}.  The refined knot sequence
$\bs{\xi}_r = \bs{\xi} \cup \zeta$ produces a control polygon with vertices on
...
\begin{Bsproperty}[Distance Between Spline Functions and Control Polygons]
  ...
  in Figure~\ref{fig:knot-operations-refine-graphic}) is that as
...
\ref{Bsproperty:convergence} with mathematical rigor.  The important information
to understand is that for large $\card{\bs{\xi}}$ the control polygon itself
is sufficient for estimation of $f,$ i.e., $\CP \bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta}
\left( x \right) \approx f\left( x \right).$  Such a calculation is
computationally more efficient than $\bs{B}_{, \bs{\xi}} \left( x \right) \bs{\theta}
\left(x\right)$ and thus has been used extensively in computer graphics and
