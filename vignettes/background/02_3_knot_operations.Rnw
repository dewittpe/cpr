...
# if you really want the W matrix you can get it from
# cpr::insertion_matrix()
W <- cpr::insertion_matrix(zeta, xi)
all.equal(target = W %*% theta, current = theta_r)

# build the two control polygons.  
# They will be plotted in Figure \ref{fig:background_refinement_plot}
cp_init <- cpr::cp(bmat, theta)
cp_r    <- cpr::cp(bmat_r, theta_r)
@

<<background_refinement_plot, eval = FALSE>>=
# Plot the two control polygons, see Figure \ref{fig:background_refinement_plot}
plot(cp_init, cp_r, show_spline = TRUE, color = TRUE) + 
...
\CP_{k, \bs{\xi}_r, \bs{\theta}_r} \left(\xi_{rj}^{*} \right) = 
\CP_{k, \bs{\xi},   \bs{\theta}} \left(\xi_{rj}^{*}\right) 
\quad \forall j.
\end{equation}
\end{Bsproperty}
...
$\bs{B}_{k, \bs{\xi}} \left( x \right) \bs{\theta} \in \mathbb{S}_{k, \bs{\xi}}$ there is no
guarantee that
$\bs{B}_{k, \bs{\xi}} \left( x \right) \bs{\theta}$ would also be in $\mathbb{S}_{k, \bs{\xi}_c}.$ 
If $\xi_j$ is removed from $\bs{\xi},$ i.e., $\bs{\xi}_c = \bs{\xi} \backslash
\xi_j,$ then the only way the
spline function to be unaltered is if 
$ \left\{ \xi_{i}^{*}, \theta_i \right\}_{i = j-3}^{j-1}$
$\CP_{k, \bs{\xi} \bs{\theta}}$ are on $\CP_{k, \bs{\xi}_c, \bs{\theta}_c}.$
That said, given $\bs{\xi}$ and $\bs{\theta}$  removing one of the internal
knots, $\xi_j, j = k + 1, k + 2, \ldots, k + l,$ we approximate $\bs{\theta}_c$
for $\CP_{k, \bs{\xi}_c, \bs{\theta}_c}$ based on the
...
in Figure~\ref{fig:background_coarsening_xi_graphic}.  The initial control
polygon and spline function are the same as in
Figures~\ref{fig:background_refinement_plot}.
The control polygon labeled \code{cp\_r} is the result of
...
about the knot removed. The first and last three control vertices have the same
coordinates for both control polygons. This also means that the spline functions
are the same within the parts of the hull which are common between the two
control polygons.

<<background_coarsening_xi>>=
# Parts need to build Figure \ref{fig:background_coarsening_xi_graphic}
# Construct the coarsened knot sequence, basis, and ordinate
xi_c <- xi[-which(xi == 1.5)]
bmat_c <- cpr::bsplines(xvec, iknots = iknots[-which(iknots == 1.5)])
theta_c <- cpr::coarsen_ordinate(1.5, xi_c, theta)

# Generate the control polygon with the coarsened knot sequence and ordinates.
cp_c <- cpr::cp(bmat_c, theta_c)

# if you want to see the (W^T W)^(-1) W^T matrix use the following
W <- cpr::insertion_matrix(1.5, xi_c)
all.equal(solve(t(W) %*% W) %*% t(W) %*% theta, theta_c)
@

\begin{figure}
  \centering
<<background_coarsening_xi_graphic, echo = FALSE, fig.width = 6, fig.height = 4>>=
plot(cp_init, cp_c, show_spline = TRUE, color = TRUE) + 
ggplot2::theme(legend.position = "bottom") 
@
\caption{The initial control polygon and the control polygon resulting from the
  knot sequence $\bs{\xi} \backslash 1.5.$
\label{fig:background_coarsening_xi_graphic}}
\end{figure}
...
\subsubsection{Coarsen and Reinsert a Knot}
...
$\CP_{k, \bs{\xi} \bs{\theta}}$ and $\CP_{k, \bs{\xi},
\bs{\theta}_{cr}}$ will
...
Figure~\ref{fig:background_reinsertion_graphic}.
...
<<background_reinsertion_setup>>=
# Estimate the control polygon ordinates.  Note that the knot sequence to pass
# into the cpr::hat_ordinate() function is the *coarsened* knot sequence.  While
# it might seem reasonable to pass xi and not xi_c into this function, the
# use of xi_c was selected to stay consistent with the notation presented above.
theta_cr <- cpr::hat_ordinate(1.5, xi_c, theta)

# The control polygon with the estimated ordinates uses the original bmat; the
# knot sequence and order are the same for cp_init and cp_cr, only the ordinates
# have changed.
cp_cr <- cpr::cp(bmat, theta_cr)

# To get at the W (W^T W)^(-1) W^T matrix
W <- cpr::insertion_matrix(1.5, xi_c)
all.equal(W %*% solve(t(W) %*% W, tol = 0) %*% t(W) %*% theta, theta_cr)
@

\begin{figure}
  \centering
<<background_reinsertion_graphic, echo = FALSE, fig.width = 6, fig.height = 4>>=
plot(cp_init, cp_c, cp_cr, show_spline = TRUE, color = TRUE) + 
ggplot2::theme(legend.position = "bottom") 
@
\caption{The initial control polygon, the control polygon resulting from the
  knot sequence $\bs{\xi} \backslash 1.5,$ and the control polygon and spline
  resulting for re-inserting the knot at 1.5 into the $\bs{\xi}.$  
\label{fig:background_reinsertion_graphic}}
\end{figure}

The control polygon resulting from re-inserting 1.5 into the coarsened knot
sequence generates a spline function that is equivalent to the spline function
of the coarsened sequence: a results of the insertion being done via Boehem's
method.  The control vertices of \code{cp\_cr} are contained on \code{cp\_c}.
The abscissa of the control vertices of \code{cp\_init} and \code{cp\_cr} are the
same because the knot sequence is the same.  The ordinates differ at $k + 1 = 5$
locations.  It is this difference in the ordinates which is used to gain
inference on the inference of any particular knot.
