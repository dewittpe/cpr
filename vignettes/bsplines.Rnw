...
\subsection{Building B-spline basis}
Build B-spline basis via \code{bsplines}.
<<>>=
str(bsplines)
@
<<include = FALSE>>=
...
<<example_bmat>>=
x <- seq(0, 6, length = 500) # will be used again
bmat <- bsplines(x      = x,
...
\subsection{Plotting B-spline Basis}
The \pkg{cpr} package provides an plotting
method, built on top of \pkg{ggplot2}~\citep{R-ggplot2},
for \code{cpr\_bs} objects.
<<label = "plot_bmat_basis", eval = FALSE>>=
plot(bmat, show_x = TRUE) # Figure \ref{fig:basis_plot}
@

\subsection{Spine Function}
As noted previously, the B-spline basis is a partition of unity over the support
of $\bs{x},$
and the span of the basis defines a spline
space $\mathbb{S}_{k, \bs{\xi}} = \text{span } \bs{B}_{k, \bs{\xi}}.$
...
An example spline function using the \code{bmat} is shown in
Figure~\ref{fig:spline_function}.


\begin{figure}
  \centering
  \begin{subfigure}[t]{0.48\textwidth}
<<label = "plot_bmat_basis", eval = TRUE, echo = FALSE, results = "hide">>=
@
...
plot(cp(bmat, theta), show_spline = TRUE, show_xi = FALSE) +
  ggplot2::theme(legend.position = "none")
@
    \caption{\label{fig:spline_function}}
  \end{subfigure}
    \caption{\label{fig:basis_and_spline}}
\end{figure}

\section{Control Polygons}
Control polygons are strong convex hulls about a spline function.  The control
polygon for a spline
$\bs{B}_{k, \bs{\xi}} \left(x \right) \bs{\theta}_{\bs{\xi}},$
is
$\CP_{k, \bs{\xi}, \bs{\theta}_{\bs{\xi}}},$
where
\begin{equation}
...
of the spline function are exaggerated by the control polygon.
