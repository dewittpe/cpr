...
// [[Rcpp::export]]
Rcpp::List bspline__impl(arma::vec x, unsigned int j, unsigned int order, arma::vec knots) { 
  ...
  return Rcpp::List::create(
      Rcpp::Named("bspline") = arma2vec(BJ.get_Bj()),
      Rcpp::Named("order")   = order,
      Rcpp::Named("data")   = arma2vec(x),
      Rcpp::Named("xi") = arma2vec(BJ.get_xi())
      );
}
...
Rcpp::NumericMatrix bbasis__impl(arma::vec x, arma::vec iknots, arma::vec bknots, unsigned int k) { 
  ...
  out.attr("xi_star") = arma2vec(greville_sites(knots, k));
