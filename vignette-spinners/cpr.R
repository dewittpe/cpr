...
#' distribuiton with one degree of freedom,
#' $$ w_j \sim \chi_{1}^{2}.$$
#'
#' Now, if we are interested in removing the knot with the lowest influnce we
#' are interestd in the the minimum.  So the hypothesis test we are actually
#' interested in is
...
#' probability of the rank order statisitic exceeding the observed value.
summary(influence_of_iknots(initial_cp))
...
#'
#' The third_reduction_cp has the first set of internal knots where the
#' minimumal influence weight is significantly different from zero.
#'
